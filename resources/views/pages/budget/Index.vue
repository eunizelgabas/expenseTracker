<template layout="default">
    <Head title="Income | Expense Tracker"/>
    <div>
        <ConfirmDialog v-if="showConfirm"
            title="Delete Budget?"
            message="Are you sure about deleting this budget?"
            @cancel="cancelDelete()"
        ...
        <div v-if="$page.props.flash.error" class="absolute top-20 right-1 p-4 bg-green-300 border border-gray-300 rounded-md shadow-md">
        ...
        <div class="bg-white">
            <div class="pt-0 pr-4 pb-0 pl-4 mt-0 mr-auto mb-0 ml-auto max-w-7xl sm:px-6 lg:px-8">
                <div class="flex justify-between">
                    ...
                    <Link href="/budget/create" class="btn-primary mb-2" >Add Income</Link>
                </div>
                <div class="pt-0 pr-4 pb-0 pl-4 mt-0 mr-auto mb-0 ml-auto max-w-4xl sm:px-6 lg:px-8">
                    <div class="pt-0 pr-4 pb-0 pl-4 mt-0 mr-auto mb-0 ml-auto sm:flex sm:items-center sm:justify-between">
                        <div>
                            <p class="text-xl font-bold text-gray-900">List of Income</p>
                        ...
                        <div class="mt-4 mr-0 mb-0 ml-0 sm:mt-0">
                            <p class="sr-only">Search Income</p>
                            <div class="relative">
                                ...
                                <input placeholder="Search Income" type="search" class="border block pt-2 pr-0 pb-2 pl-10 w-full py-2
                    ...
                    <div class="shadow-xl mt-8 mr-0 mb-0 ml-0 pt-4 pr-10 pb-4 pl-10 flow-root rounded-lg sm:py-2">
                        <div class="pt--10 pr-0 pb-10 pl-0">
                            <div class="pt-5 pr-0 pb-0 pl-0 mr-0 mb-0 ml-0" v-for="bud of budgets.data" :key="bud.id">
                                <div class="sm:flex sm:items-center sm:justify-between sm:space-x-5">
                                    <div class="flex items-center flex-1 min-w-0">
                                        <img src="asset/budget.png"
                                            class="flex-shrink-0 object-cover rounded-full btn- w-10 h-10"/>
                                        <div class="mt-0 mr-0 mb-0 ml-4 flex-1 min-w-0">
                                            <p class="text-lg font-bold text-gray-800 truncate">{{ bud.description }}</p>
                                            <p class="text-green-600 text-md">${{ bud.amount }}</p>
                                            <p class="text-gray-600 text-md">{{ formattedDate(bud.created_at) }}</p>
                                        </div>
                                    </div>
                                    <div class="mt-4 mr-0 mb-0 ml-0 pt-0 pr-0 pb-0 pl-14 flex items-center sm:space-x-6 sm:pl-0 sm:justify-end
                                        sm:mt-0">
                                        <Link :href=" '/budget/edit/' + bud.id" class="bg-green-600 pt-2 pr-6 pb-2 pl-6 text-lg font-medium text-gray-100 transition-all
                                            ...
                                            duration-200 hover:bg-red-700 rounded-lg" @click="remove(bud)">Delete
                                        </button>
                                    </div>
                                </div>
...
let selectedBudgetForDelete = null
...
function remove(bud){
 ...
 function confirmDelete(){
    form.delete('/budget/' +selectedBudgetForDelete.id);
    showConfirm.value = false;
 }
